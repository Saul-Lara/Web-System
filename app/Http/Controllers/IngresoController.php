...
class IngresoController extends Controller
{
    ...
    public function show($id){
        ...
        $ingreso = DB::table('ingresos as i')->join('personas as p' , 'i.proveedor', '=' , 'p.idpersona')
                                             ->join('detalles-ingreso as di' , 'i.idingreso', '=' , 'di.idingreso')
                                             ->select('i.idingreso','i.fecha_hora','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.num_comprobante','i.impuesto','i.estado',DB::raw('sum(di.cantidad*precio_compra) as total'))
                                             ->where('i.idingreso','=', $id)
                                             ->first();

        $detalles = DB::table('detalle_ingreso as d')->join('articulo as a', 'd.idarticulo','=','a.idarticulo')
